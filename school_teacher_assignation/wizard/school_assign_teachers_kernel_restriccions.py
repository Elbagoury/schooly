...
def professors_candidats_ordenats(data_by_teacher,
    ...
    tcd_list_ordered = sorted(
        filter(lambda x,dbt=data_by_teacher: x and x['id'] in dbt,
               teacher_course_data_list),
    ...
    for tcd in tcd_list_ordered:
        ...
        if criteri_teachers_suficients(max_hours, len(new_tcd_list)):
            break
    return new_tcd_list
...
def assigna_classes(clss, d_x_tch, l_tch_crs_d,
                    ...
                    max_sol, p_cls=0, 
                    afinament = 0.1, time_limit=None):

    l_sols_to_ret = []
    cls_dur = clss[p_cls]['duration']

    def criteri_teachers_suficients(h_max, num_tch,
            h_min=total_hours*afinament,
            num_tch_min=(len(clss) - p_cls)):
        return h_max > h_min and \
               num_tch > num_tch_min
...
def _assigna_cursos(courses_punter, cursos_list_orig,
                   ...
                   teachers_data_by_course, max_solutions, time_limit=None):
    ...
    for (value, assignacio) in sols:
        ...
        for cp in range(len(cursos_list)):
            ...
            _acabat, value_assignacio_list = _assigna_cursos(
                ...
                max_solutions, time_limit=time_limit
            )
            ...
            for (value2, assignacio_by_course) in value_assignacio_list:
                assignacio_by_course[course_id] = assignacio # copia ?
...
def assigna_cursos(cursos_list_orig,
    ...
    if MP:
        ...
        res = [_assigna_cursos(courses_punter,
                              cursos_list_orig,
                              total_classes_hours,
                              classes_data_by_course,
                              total_teachers_hours,
                              ...
                              teachers_data_by_course,
                              max_solutions,
                              time_limit) for courses_punter in range(len(cursos_list_orig))]
    acabat = 0.0
    for _acabat, value_assignacio_list in res:
        acabat += _acabat / len(cursos_list_orig)
...
if __name__ == '__main__':
    ...
    solucions = (acabat, llista_de_solucions)
